...
pub fn token_breakdown(usage: &[Entry]) -> TokenBreakdown {
    ...
    if result.cache_hit == 0 && result.cache_miss == 0 {
        ...
        if detailed != prompt_total {
            log::warn!(
                "PROMPT_TOKEN({prompt_total}) 与缓存明细({detailed})不一致，已按明细口径计入 total"
            );
        }
    }
    result
...
///
/// 金额用 `f64` 而非整数分（P2-02 取舍）：平台返回的就是小数字符串，本应用只做
/// 展示级汇总（元、两位小数），不做财务入账；改整数分需同步改接口反序列化与前端
/// 全部 `fmtMoney` 调用链，收益不抵成本。极端量级（>2^53 最小单位）的精度损失可接受。
